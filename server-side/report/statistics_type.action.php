...
$c1="3 or incomming_call.call_type_id=0";
...
case $text[0]:
	...
	case $text[1]:
			...
			AND DATE(`incomming_call`.`date`) <= '$end' AND department.`name`='$name' and (incomming_call.call_type_id=$c1) GROUP BY c_name)*100,2),'%')
			FROM incomming_call
